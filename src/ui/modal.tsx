...
interface ModalProps {
  ...
  size?: "default" | "lg";
  /**
   * id of the element to mark `inert` while the modal is open (see the
   * screen-reader virtual-cursor note below). Defaults to `"app-root"`.
   */
  rootInertId?: string;
}
...
export function Modal({
  ...
  // Stash the latest onClose / initialFocusRef / rootInertId in refs so the
  // open-effect's dep array can stay limited to [open]. Without this, every
  // parent re-render that changes the inline `onClose` arrow (which is
  // essentially every re-render at every existing call site) would tear down
  // and re-run the effect, restoring focus to the opener and then
  // re-focusing into the dialog — a focus-thrash that drops keystrokes and
  // breaks IME composition while the user is typing in a Modal-hosted form.
  const onCloseRef = useRef(onClose);
  const initialFocusRefRef = useRef(initialFocusRef);
  const rootInertIdRef = useRef(rootInertId);
  useEffect(() => {
    onCloseRef.current = onClose;
    initialFocusRefRef.current = initialFocusRef;
    rootInertIdRef.current = rootInertId;
  });
  ...
  useEffect(() => {
    ...
    const inertRootId = rootInertIdRef.current ?? "app-root";
    document.getElementById(inertRootId)?.setAttribute("inert", "");
    ...
    return () => {
      ...
      document.getElementById(inertRootId)?.removeAttribute("inert");
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" data-modal-overlay>
      <div
        className="fixed inset-0 bg-chat-page/60 backdrop-blur-sm"
        onClick={onClose}
        aria-hidden
      />
      <div
        ...
        tabIndex={-1}
        className={cn(
          "relative z-[1] flex max-h-[85vh] w-full flex-col rounded-lg border border-chat-rule bg-chat-surface shadow-xl [outline:none]",
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-chat-rule p-4">
